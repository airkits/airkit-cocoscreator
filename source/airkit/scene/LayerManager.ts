...
namespace airkit {
    ...
    export class LayerManager extends Singleton {
        ...
        public static resize(): void {
            Log.info("LayerManager Receive Resize {0} {1}", cc.director.getWinSize().width, cc.director.getWinSize().height);
            ...
            for (let i = 0; i < needUpChilds.length; i++) {
                ...
                for (let j = 0, l = layer.numChildren; j < l; j++) {
                    var child: fgui.GComponent = layer.getChildAt(j) as fgui.GComponent;
        ...
        public static addBg(url: string): fgui.GLoader {
            ...
            if (this.bgLayer.numChildren) child = this.bgLayer.getChildAt(0) as fgui.GLoader;
            else {
