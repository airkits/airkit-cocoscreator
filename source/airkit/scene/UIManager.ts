...
// import { eUIQueueType, eAligeType } from "../common/Constant";
    ...
    export class UIManager extends Singleton {
        ...
        public show(id: number, ...args: any[]): Promise<any> {
            return new Promise((resolve, reject) => {
                ...
                assert(conf != null, "UIManager::Show - not find id:" + conf.mID)
                ...
                v.loadResource(ResourceManager.DefaultGroup, clas)
                    .then(p => {
                        let layer: fgui.GComponent = LayerManager.getLayer(conf.mLayer)
                        layer.addChild(p)
        ...
        public close(id: number, animType: number = 0): Promise<any> {
            return new Promise((resolve, reject) => {
                ...
                //获取数据
                let loader_info: UIConfig = this._dicConfig.getValue(id)
        ...
        public clearPanel(id: number, panel: IUIPanel, loader_info: any): boolean {
            //销毁或隐藏
        ...
        public singleToast(
            ...
            return new Promise((resolve, reject) => {
                ...
                view.setScale(0.1, 0.1)
                //fgui坐标转化有问题，临时处理一下
                if (x == null) x = target.width / 2.0 - target.pivotX * target.width
                if (y == null) y = target.height * 0.382 - target.pivotY * target.height
                ...
                let scale = 1.0
                tu.to({ scaleX: scale, scaleY: scale, alpha: 1.0 }, duration, inEase).to(
                    { alpha: 0.4 },
                    duration * 0.7,
                    outEase,
        ...
        public addUIConfig(info: UIConfig): void {
            if (this._dicConfig.containsKey(info.mID)) {
                Log.error("UIManager::Push UIConfig - same id is register:" + info.mID)
                return
            }
...
*/
    export class AlertInfo {
        ...
        constructor(callback: Function, title: string = "", content: string, tips: string = "", buttons: any = {}, param: any = null) {
    ...
    class UIQueue {
        ...
        public empty(): boolean {
            if (this._currentUI > 0 || this._listPanels.length > 0) return false
            return true
        }
        /**
         * 判断是否弹出下一个界面
        ...
        private registerEvent(): void {
            EventCenter.addEventListener(EventID.UI_CLOSE, this, this.onUIEvent)
        }

        private unRegisterEvent(): void {
            EventCenter.removeEventListener(EventID.UI_CLOSE, this, this.onUIEvent)
        }
