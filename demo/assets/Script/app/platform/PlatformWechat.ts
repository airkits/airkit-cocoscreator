module app.platform {
    export class PlatformWechat {
        ...
        public setApiServer(url: string): void {
            PlatformWechat.AuthServer = url;
        }
        public init(t: ePlatform): void {
            //小游戏处理暂停
            if (!this.isWXEnv) {
                return;
            }
            ...
            //   xqwxgamesdk.initSetting("https://passport.2345.com", "xcx_xqgy", "wx9b8a79ea714d36a5", "plg6PAJniPHe5Lulz80Z9EyG0bAyhwM7");
        }
        ...
        public createAuthButton(pos: cc.Vec2, size: cc.Size, onSuccess: Function, onFailed: Function): any {
            ...
            button.onTap((res: any) => {
                ...
                if (res.errMsg.indexOf("auth deny") > -1 || res.errMsg.indexOf("auth denied") > -1) {
                    ...
                    that.getUserInfo(this.pushData)
                        .then((v) => {
                            this.hideLoading();
                            onSuccess(v);
                            return true;
        ...
        login(): Promise<PlatformUser> {
            ...
            // return new Promise((resolve, reject) => {
            //     let that = this;
            //     xqwxgamesdk.loginCheck((res: any) => {
            //         console.log(res);
            //         switch (+res.rescode) {
            //             case xqwxgamesdk.eResCode.SUCCESS: {
            //                 this.gameAuth(res.sData)
            //                     .then((v) => {
            //                         resolve(v);
            //                     })
            //                     .catch((e) => {
            ...
            //                 break;
            //             }
            //             case xqwxgamesdk.eResCode.NET_ERROR: {
            //                 wx.showModal({
            //                     title: "网络异常",
            //                     content: "网络异常，请重试～", ///重试  退出游戏
            //                     showCancel: true,
            //                     cancelText: "退出游戏",
            //                     confirmText: "重试",
            //                     success: function (result: any) {
            ...
            //                         if (result && result.errMsg == "fail cancel") {
            ...
            //                 break;
            //             }
            //             case xqwxgamesdk.eResCode.CHECK_FAIL_NEED_LOGIN_CODE: {
            //                 this.loginWX()
            //                     .then((v) => {
            //                         resolve(v);
            //                     })
        ...
        loginWX(): Promise<PlatformUser> {
            return new Promise((resolve, reject) => {
                let that = this;

                // let obj = {
                //     success: (res: _loginSuccessObject) => {
                // let user = new PlatformUser();
                // user.data = {
                //     code: res.code,
                // };
                //rescode:success||net_error||need_redo_login_code||need_to_auth
                // xqwxgamesdk.loginCode(res.code, (ret) => {
                //     console.log(ret);
                //     this.pushData = ret.pushData;
                //     switch (+ret.rescode) {
                //         case xqwxgamesdk.eResCode.SUCCESS: {
                //             that.gameAuth(ret.sData, user)
                //                 .then((v) => {
                //                     resolve(user);
                //                 })
                //                 .catch((e) => {
                ...
                //         }
                //         case xqwxgamesdk.eResCode.NET_ERROR:
                //         case xqwxgamesdk.eResCode.NEED_REDO_LOGIN_CODE: {
                //             wx.showModal({
                //                 title: "网络异常",
                //                 content: "网络异常，请重试～", ///重试  退出游戏
                //                 showCancel: true,
                ...
                //             break;
                //         }
                //         case xqwxgamesdk.eResCode.NEED_TO_AUTH: {
                //             reject("newuser");
                //             break;
                //         }
                //         default:
        ...
        gameAuth(sData: string, user?: PlatformUser): Promise<any> {
            return new Promise((resolve, reject) => {
                let that = this;
                //let data = {
                // code: user.data["code"],
                //     platform: "wechat",
                //     //    iv: user.data["iv"],
                // //  encrytedData: user.data["encrytedData"]
                // };
                user = user || new PlatformUser();
                // let cookieI = 'aT01MDAwNDY2NTgmdT01MDAwNDY2NTgmbj0lQ0ElRDYlQkIlRkElRDMlQzMlQkIlQTdfMzc1NDU5Nzc0MiZtPTMwMCZ0PTE1ODc4NjA5MjguNDYyMDAwMCZzPWNhYWVlMjUwY2ZlMWZiNzVhZTI4ZDAyMmVhYjI3NDBjJnY9MS4x';
                // this.setCookieI(cookieI);
                // resolve(user);
                // wx.request({
                //     url: net.param.baseUrl + "orchard/AppletApi/Init",
                //     method: "POST",
                //     header: { "Content-Type": "application/x-www-form-urlencoded" } as Object,
                //     data: { I: sData, inviterId: app.model.wxXqInviteId },
                //     success: function (res) {
                //         let data = res.data;
                //         if (+data["code"] == 200) {
                //             let cookieI = data["data"]["I"];
                //             that.setCookieI(cookieI);
                //             let source = data["data"]["data"]["user_source"];
                //             let guide = data["data"]["data"]["novice_guide_step"];
                //             // if(+source == 0  && guide == 0){
                //             //     showMsg("你已经领取过该红包了");
                //             // }
                //             user.data["source"] = +source;
                //             user.data["guide"] = +guide;
                //             // me.gold = +data["data"]["data"]["user_gold"];
                //             //me.needShowWxDuanwuBtn = data["data"]["activity_share"] ? parseInt(data["data"]["activity_share"]) : 0;
                //             //me.hasDrinkWater = data["data"]["drink_water"] ? parseInt(data["data"]["drink_water"]) : 0;
                //             resolve(user);
        ...
        public getUserInfo(pushData: string): Promise<PlatformUser> {
            return new Promise((resolve, reject) => {
                let that = this;
                wx.getUserInfo({
                    ...
                    success: function (res) {
                        ...
                        // xqwxgamesdk.loginAuth(pushData, res.encryptedData, res.iv, (ret) => {
                        //     switch (+ret.rescode) {
                        //         case xqwxgamesdk.eResCode.SUCCESS: {
                        //             that.gameAuth(ret.sData, user)
                        //                 .then((v) => {
                        ...
                        //             break;
                        //         }
                        //         case xqwxgamesdk.eResCode.AUTH_FAIL_NEED_LOGIN_CODE: {
                        //             reject();
                        //             break;
                        //         }
                        //     }
