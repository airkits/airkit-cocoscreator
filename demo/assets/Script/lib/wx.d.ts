...
declare const wx: {
    uma: {
        trackEvent(name: string, object?: any);
    };
    /**
     ...
     * 监听小游戏回到前台的事件
     */
    onShow(callback: (obj: any) => void): void;
    /**
     * 取消监听小游戏回到前台的事件
     */
     ...
     */
    onShareAppMessage(callback: (a?: any, b?: any, c?: any) => void): void;
    onShareTimeline(callback: (a?: any, b?: any, c?: any) => void): void;
    /**
     ...
     */
    offKeyboardConfirm(): void;
    /**
     * 监听监听键盘收起的事件
     */
    onKeyboardComplete(callback: (res: any) => void): void;
    /**
     * 取消监听监听键盘收起的事件
     */
    offKeyboardComplete(): void;
    /**
     * 显示键盘
     */
    showKeyboard(object: {
     ...
     * 插屏广告
     */
    createInterstitialAd(obj: any): any;
    /**
     * banner广告
     */
    ...
    navigateToMiniProgram(object: {
        ...
        extraData?: any; //		否	需要传递给目标小程序的数据，目标小程序可在 App.onLaunch，App.onShow 中获取到这份数据。如果跳转的是小游戏，可以在 wx.onShow、wx.getLaunchOptionsSync 中可以获取到这份数据数据。
    ...
    createGameIcon(object: {
        ...
        count: 1;
        style: Array<{
            appNameHidden?: boolean; //		是	游戏名称是否隐藏
            color?: string; //	是	游戏名称的颜色色值
            size?: number; //		是	游戏icon的宽高值
            borderWidth?: number; //		是	游戏icon的border尺寸
            borderColor?: string; //	是	游戏icon的border颜色色值
            left?: number; //		是	游戏icon的X轴坐标
            top?: number; //		是	游戏icon的Y轴坐标
        }>;
    });
};
...
declare interface Canvas {
    ...
    getContext(
        contextType: string,
        contextAttributes: { antialias: boolean; preserveDrawingBuffer: boolean; antialiasSamples: number }
    ): WXRenderingContext;
    /**
...
/**
 * 通过 Canvas.getContext('2d') 接口可以获取 CanvasWXRenderingContext2D 对象。CanvasWXRenderingContext2D 实现了 HTML The 2D rendering context 定义的大部分属性、方法。通过 Canvas.getContext('webgl') 接口可以获取 WebGLWXRenderingContext 对象。 WebGLWXRenderingContext 实现了 WebGL 1.0 定义的所有属性、方法、常量。
 * 2d 接口支持情况
