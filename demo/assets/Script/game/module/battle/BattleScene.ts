...
export default class BattleScene extends UIBattleScene {
    ...
    onEnable(): void {
        ...
        ws.initServer('ws://127.0.0.1:12080?UID=' + this._uid + '&token=aaaa', PBMsg)
            .then((result) => {
                ws.request({ uid: this._uid, msgID: c2s.MessageCmd.JOIN_ROOM, typeUrl: './c2s.JoinRoomReq', body: { uid: this._uid, roomID: this._roomID }, msgType: cs.MessageType.Request }).then(
                    (v) => {
                        console.log(v)
                        if (v && v.body) {
    ...
    public static res(): Array<ak.Res> {
        return ak.Utils.buildRes(UIBattleScene.ResMap, UIResMap.ResMap)
    }
    ...
    protected eventMap(): Array<any> {
        return [[this.btnBack, fgui.Event.CLICK, this.onBtnBackClick]]
    }
